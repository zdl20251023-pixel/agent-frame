...
- 流程图负责表达路径，不要把所有依据塞进图里。
- 说明必须一行一个核心观点，避免大段文字影响阅读。
- 每条说明后面直接带官方依据或主观推断标记。
- 不再单独拆“官方依据”或“主观推断”小节，避免冗余。
- 官方依据使用可点击 Markdown 链接，例如：参考：[Redis 官方 ZRANGE 文档](https://redis.io/docs/latest/commands/zrange/)
...
- 写流程必须用 Mermaid 先表达写入路径、事务边界、缓存处理和补偿路径。
- 说明里一行只写一个判断，避免把 MySQL、Redis、补偿、风险混成一整段。
- 每条说明后面直接带官方依据或主观推断标记。
- 如果是“先写 MySQL、事务提交后再删/改 Redis”这类工程策略，必须标记 **标记：主观推断**。
- 如果引用 Redis 命令能力，例如 `SET`、`DEL`、`EXPIRE`、`ZADD`，必须使用可点击 Redis 官方文档链接。
...
- 异常处理也要画流程图，优先表达 Redis 不可用、MySQL 慢、缓存重建失败、并发 miss、key 被淘汰后的处理路径。
- 说明里一行只写一个异常判断或降级策略。
- 每条说明后面直接带官方依据或主观推断标记。
- 降级策略、是否返回旧值、是否返回空结果、是否限流，通常属于工程判断，必须标记 **标记：主观推断**。
- 如果引用 Redis TTL、过期、淘汰、锁相关命令能力，必须使用可点击 Redis 官方文档链接。
