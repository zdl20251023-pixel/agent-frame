...
| score 规则不清楚 | 提前定义同分排序、累加/覆盖、封顶规则 |
...
前面重点讲了 String、Hash、ZSET；下面几个场景不展开，只看 **业务特征和 Redis 选择是否匹配**。
...
| 学习行为 | Stream | 行为持续产生，适合异步消费 | 只写 Stream 不写事实源，或消费不做幂等 |
...
**Stream 适合轻量异步事件流，但关键学习结果仍然要落 MySQL 或日志系统。**

---

### 5.5 活动趋势：Time series
