...
**本系统关键业务事实保存在 MySQL；Redis 主要承担缓存、性能视图和异步处理。**
...
热门课程缓存失效时，核心不是让每个请求都重建缓存，而是通过重建锁或 singleflight，让少量请求回源 MySQL，其他请求等待或复用结果。
...
```json
{
  ...
  "status": "learning",
  "last_learned_at": "2026-07-05T12:00:00Z",
  "learned_seconds": 3600,
  "completed_lessons": 8
}
```
...
事实源:
MySQL 保存可恢复的学习进度、完成记录等关键事实
```


注意：

**Redis Hash 只做学习进度缓存，MySQL 保存可恢复的进度事实和最终学习结果。**
...
flowchart TD
    ...
    J["用户学习进度变化"] --> K["先写 MySQL 进度事实"]
    K --> L{"MySQL 写入成功？"}
    L -->|否| M["返回更新失败"]
    L -->|是| N["HSET 或 HINCRBY 更新 Redis Hash"]
    N --> O["设置或刷新 TTL"]
    O --> P["返回更新成功"]
```
...
2. **进度变化先落 MySQL**：保证 Redis 丢失后，学习进度仍然可以恢复。
3. **Redis 做局部更新缓存**：MySQL 成功后，再用 `HSET` / `HINCRBY` 更新指定字段。
...
| 风险 | 兜底 |
|---|---|
| 把学习进度事实只放 Redis | 可恢复进度和最终完成记录必须落 MySQL |
| Hash 字段无限增长 | 只放稳定字段 |
...
**`ZREVRANK` 的时间复杂度是 `O(log(N))`，其中 `N` 是有序集合中的元素数量。**

**本案例只演示按分数排序；同分规则需要由实际业务统一定义，最终榜以 MySQL 排序规则为准。**
...
**消费组**：多个消费者组成一个小组一起处理同一个队列里的消息，新消息通常先分配给组内某一个消费者处理。
...
正常首次投递时，同一条消息只分配给其中一个 Consumer，组内是**竞争消费、分摊负载**；失败接管或重试时可能再次处理，因此消费者必须幂等。  
...
**MySQL 保存关键学习事实，消息队列只负责触发后置处理；消息发送失败时，通过补偿任务重新投递。**
...
同一消费组内，多个 Consumer 竞争处理新消息；失败接管或重试时可能重复处理，因此消费者必须幂等。
...
普通处理完成后使用 `XACK` 确认；Redis 8.2.0 起，也可以使用 `XACKDEL` 在确认当前消费组的同时，按消费组状态尝试删除消息。多个消费组场景可使用 `ACKED`，只有消息已被所有消费组读取并确认后才删除：
...
| 活动参与 | Set | 判断是否参与，适合去重 | 只靠 Redis 防重复，没有 MySQL 唯一约束 |
| 活动趋势 | Time series | 指标是时间戳 + 数值，适合看趋势 | 把它当完整监控、日志或链路追踪系统 |

补充说明：

Bitmap 和 Bitfields 的相同点：**它们本质上都是基于 Redis String 的二进制位操作能力，都是把数据压缩存到一串 bit 里面。**
...
```text
QPS：系统每秒实际处理的请求数量，用于观察流量和吞吐，不能单独代表系统承载能力
