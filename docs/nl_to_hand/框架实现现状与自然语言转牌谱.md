...
> 说明：本文档描述当前仓库的实际实现状态，不是早期设计稿。自然语言转牌谱已经从“孤立业务工具”推进到“能力路由 + 专用 Agent + Tool Bridge + ToolInvocation 恢复 + 异步修复 Worker + Artifact 状态化”的生产化雏形。
...
| Tool Bridge | 已支持通用 `AgentToolDefinition` 和 `nl_to_hand` 桥接 | `apps/api/src/ai/tools/` |
...
| 前端牌谱编辑 | 已支持“基于此牌谱继续修改” | `apps/web/src/features/artifacts/HandHistoryPanel.tsx` |
...
### 问题二：异步修复已经拆出，但前端状态体验还不完整

后端已经有 `waiting_repair`、`agent_tasks` 和 `NlToHandRepairWorker`，但前端还没有完整展示：
...
当前用户能看到 draft Artifact，但后台修复任务进度、失败原因、成功后新版本提示还需要更清晰的 UI 表达。

建议：

- `HandHistoryPanel` 显示 `handHistoryState.status`。
- 前端轮询或订阅 `AgentTask` / `ToolInvocation` 状态。
- 当后台修复成功后突出显示新 `ArtifactVersion`。
- 当后台修复失败时显示“需要用户补充”的明确入口。

### 问题三：插件发现与运行时执行仍未完全统一

当前 `PluginRegistry` 已能注册 Agent、Tool、Workflow、ArtifactType 元数据；但真正执行仍主要依赖手写 `ToolRegistry`、`AgentExecutorRouter` 和容器注册。

这在 MVP 阶段可以接受，但长期会导致：

- “可发现能力”和“可执行能力”之间有重复映射。
- CapabilityRouter 随能力增加需要手动维护规则。
- 插件难以自带完整 runtime adapter。

建议后续把插件能力拆成两层：

```txt
Plugin.registerToolMetadata()
Plugin.registerToolRuntime()
Plugin.registerAgentRuntime()
...
| 后台修复前端状态 | 部分完成 | 后端有状态，前端展示仍需增强 |
| LLM 质量评测 | 未完成 | 需要 golden cases 和 eval runner |
| 插件运行时统一 | 未完成 | 元数据和执行注册仍未完全统一 |
