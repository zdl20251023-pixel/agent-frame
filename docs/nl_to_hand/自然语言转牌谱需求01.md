...
```typescript
import { generateText, LanguageModel, Output, tool, UIMessage } from 'ai'
import { z } from 'zod'
...
import {
  ...
  simulateHand,
} from '../feature/game_server/machine/simulator/hand_simulator'
  ...
  return tool({
    ...
    execute: async ({ game_hand }) => {
      ...
      try {
        ...
        // ── Phase 2 内层私有修复回路 ──────────────────────────────────────────
        // 默认使用 geminiFlashLiteModel：完整支持 generateObject 所需的 json_schema 模式
