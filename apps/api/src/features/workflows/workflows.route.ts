...
// GET  /workflows/runs           — 列出所有 WorkflowRun
...
export const workflowsRoute = new Elysia({ prefix: '/workflows' })
  ...
  // GET /workflows/runs — 列出 WorkflowRun，必须放在 /:workflowId 前避免 runs 被当成 workflowId
  .get(
    '/runs',
    async () => {
      const runs = await container.workflowStore.listWorkflowRuns()
      return {
        runs: runs
          .slice()
